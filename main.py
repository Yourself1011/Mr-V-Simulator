...
def renderFrame(rays: list[Ray], f, intro=False):
    ...
    screen.create_rectangle(
        0, screen.height // 2, screen.width, screen.height, fill="gray", tags="delete"
    )
    ...
    for i, ray in enumerate(rays):
        ...
        rayToSlice(
            i, ray, player, hit[0], textureMap[hit[2]], hit[1], darknessMultiplier
        )
    ...
    if not intro:
        ...
        if (
            round(player.x / 64) == mapInfo()[2][0] + 1
            and floor(player.y / 64) == mapInfo()[2][1]
        ):
            screen.create_text(
                screen.width / 2,
                screen.height / 2 - 10,
...
def pauseMenu(index):
    screen.tag_bind(
        "optionsButton", "<Button-1>", lambda e: optionsScreen(lambda: pauseMenu(index))
    )
...
def optionsScreen(returnFunction):
    ...
    def updateGraphics(value, returnFunction):
        ...
        player.rays = [
            Ray(fov / screen.width * i - fov / 2)
            for i in range(0, screen.width, consts.getVerticalPrecision())
        ]
        screen.delete("delete")
    ...
    screen.tag_bind(
        "graphicsButton",
        "<Button-1>",
        lambda e: updateGraphics(
            (consts.precision + 1) % len(consts.precisionPresets), returnFunction
        ),
    )
...
def introScreen():
    ...
    for i in range(100):
        ...
        screen.create_oval(x, y, x + size, y + size, outline="", fill="white")
...
def startGame(levelParam, reset=False):
    ...
    if debug:
        consts.localScreenDScale = screenD.length / (
            64 * round(5 * log(level + 0.1, 10) + 13)
        )
    ...
    while True:
        ...
        # print((1 / fps) - (time() - frameStart))
        root.after(round(1000 * max(0, (1 / fps) - (time() - frameStart))))
        totalF += 1
